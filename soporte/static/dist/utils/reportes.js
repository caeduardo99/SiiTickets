$(document).ready(function () {
          ...
          data.forEach(function (item) {
            ...
            row.addEventListener("click", function () {
              ...
              fetch(`getInfoReport/${ticketId}/`)
                ...
                .then((data) => {
                  console.log(data)
                  let proyectoinfo, infoTaskMain, infoTaskSecond, infoAgentWork, infoEnterpiseWork;
                  ...
                  hoursSuccess = data.hourWorlProject.length == 0 ? null : data.hourWorlProject[0].horasPrincipales;
                  print('horas del proyecto', hoursSuccess)
                  var fechaCreacionFormateada = formatFecha(proyectoinfo.fechaCreacion)
                  var fechaEstimadaFormateada = formatFecha(proyectoinfo.fechaFinalizacionEstimada)
                  
                  makePdf(proyectoinfo.tituloProyecto, proyectoinfo.descripcionActividadGeneral,`${proyectoinfo.NombreAgenteAdministrador} ${proyectoinfo.ApellidoAgenteAdministrador}`, fechaCreacionFormateada, fechaEstimadaFormateada, proyectoinfo.NombreCompletoSolicitante, proyectoinfo.nombreEmpresa, infoTaskMain, proyectoinfo.EstadoProyecto, proyectoinfo.horasCompletasProyecto, infoTaskSecond, infoAgentWork, infoEnterpiseWork, hoursSuccess);
                });
            });
  ...
  function makePdf(tituloProyecto,textDescripcionRequerimiento, nameAgente, fechaCreacion, fechaEstimada, solicitante, empresa, arrayTaskPrincipal, estadoProyecto, horasCompletas, arrayTaskSecond, arrayWorkAgent, arrayWorkEnterprise, hoursSuccessProyect) {
    ...
    var opcionesDeFormato = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', timeZone: 'UTC' };
    var fechaFormateadaActualReport = fechaActualReport.toLocaleString('es-ES', opcionesDeFormato);
      ...
      content: [
        ...
        },
        {
          ul: [
            ...
            `Agente Administrador del proyecto: ${nameAgente == null ? 'Sin asignar' : nameAgente}`,
            `Fecha de creación: ${fechaCreacion}`,
            `Fecha estimada de culminación: ${fechaEstimada == null ? 'Sin asignar' : fechaEstimada}`,
        ...
        },
        {
          table: {
            ...
            body: [
              [
                "Agente",
                "Horas de actividades finalizadas",
              ],
              ...(arrayWorkAgent.length > 0
                ...
                : [
                    [
                      {
                        text:
                          "Aún ningun agente ha cumplido con las tareas asignadas, contactece con el agente administrador del proyecto.",
                        colSpan: 2,
                        alignment: "center",
                      },
                      {},
          ...
          ul: [
            {
              text: [
                ...
                {
                  text: hoursSuccessProyect ? `${hoursSuccessProyect} horas` : "",
                  fontSize: 10,
                  bold: true,
                },
                {
                  text: hoursSuccessProyect ? " de actividades realizadas en el proyecto." : "",  // Evita mostrar " horas" si no hay valor
                  fontSize: 10,
                  italics: true,
                },
              ],
